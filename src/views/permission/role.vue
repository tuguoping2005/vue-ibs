<template>
  <div class="app-container">
    ...
    <el-table :data="rolesList" style="width: 100%;margin-top:30px;" border>
      ...
      >
        <!-- <template slot-scope="scope">
          {{ scope.row.actionAuth }}
        </template> -->
      </el-table-column>
      <el-table-column align="header-center" label="Description" width="220">
        <template slot-scope="scope">
    ...
    permListFormatter(row, column, cellValue) {
      ...
      for (let item of cellValue) {
        ...
        try {
          ...
          if(item.redirect==="/main"){
            str.push(item.children[0].meta.title)
          }
          str.push(item.meta.title);
          if(Array.isArray(item.children)){
            item.children.forEach((v)=>{
              str.push(v.meta.title)
            })
          }
...
*/
    ...
    handleAddRole() {
      ...
      if (this.$refs.tree) {
        //setCheckedNodes(nodes)设置目前勾选的节点
    ...
    async confirmRole() {
      ...
      const { description, key, name, actionAuth } = this.role;
